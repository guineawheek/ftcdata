...
def deploy(options):
    args = ["python3", "deploy.py", "--project", options.deploy.project]
...
@task
def install_libs():
    return # we don't have deploy_requirements atm
    sh("pip3 install -r deploy_requirements.txt -t lib")


@task
def jinja2():
    sh("python3 compile_jinja2_templates.py")
...
def lint(options):
    ...
    sh("python3 ops/linter.py {}".format(args))
...
def make_endpoints_config():
    sh("python3 lib/endpoints/endpointscfg.py get_openapi_spec mobile_main.MobileAPI --hostname tbatv-prod-hrd.appspot.com")
...
def test_fast():
    ...
    sh("python3 run_tests.py --test_pattern=test_math_*.py")
    sh("python3 run_tests.py --test_pattern=test_*helper*.py")
    sh("python3 run_tests.py --test_pattern=test_*parser*.py")
    sh("python3 run_tests.py --test_pattern=test_*manipulator.py")
    sh("python3 run_tests.py --test_pattern=test_*api.py")
    sh("python3 run_tests.py --test_pattern=test_event.py")
    sh("python3 run_tests.py --test_pattern=test_match_cleanup.py")
    sh("python3 run_tests.py --test_pattern=test_event_group_by_week.py")
    sh("python3 run_tests.py --test_pattern=test_event_team_repairer.py")
    sh("python3 run_tests.py --test_pattern=test_event_team_updater.py")
    sh("python3 run_tests.py --test_pattern=test_event_get_short_name.py")
...
def test_function(args):
    ...
    sh("python3 run_tests.py%s" % test_pattern)
